...
namespace Cgp.Infraestrutura.ServicosExternos.PersistenciaViaEntityFramework.Repositorios
{
    public class RepositorioDeUsuarios : Repositorio<Usuario>, IRepositorioDeUsuarios
    {
        ...
        public IList<Usuario> RetornarUsuariosPorFiltro(string nome, string email, int batalhao, bool ativo, bool ehAtenas, int perfil, int pagina, int registrosPorPagina, out int quantidadeEncontrada)
        {
            var query = this._contexto.Set<Usuario>()
            ...
            if (perfil > 0)
                query = query.Where(c => (int)c.PerfilDeUsuario == perfil);

            if (ehAtenas)
                query = query.Where(c => c.PerfilDeUsuario != Dominio.ObjetosDeValor.PerfilDeUsuario.Administrador);
